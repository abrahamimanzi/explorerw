...
if(Input::checkInput('id','get','1')){
    ...
    switch($request){
        ...
        default:
            ...
            $participantTable->selectQuery("SELECT * FROM `groups` WHERE `ID`=? ORDER BY `ID` DESC LIMIT 1",array($participant_ID));
            ...
            if($participantTable->count()){
                ...
                if($participant_data->ID == $participant_ID){ 
		...
		case 'payment-new':
            ...
            switch($request){
                case 'group':
                    ...
                    $accountData = array(
                        'merchant_id' => 'TESTBOK000003', // for test card
                        'access_code' => 'D4342031', // for test card
                        'secret'      => '85A3C14B3871EB7802D07072B25O959C' // for test card
                ...
                default:
                    ...
                    $discount = 0;
                    $participantTable = new \Group();
                    $participantTable->selectQuery("SELECT * FROM `groups` WHERE `ID`=? AND `payment_state`='Pending' OR `payment_state`='Cancelled' ORDER BY `ID` DESC LIMIT 1",array($participant_code));
                    ...
                    if($participant_data->citizenship_country=="Rwanda"){
                        $currency_str = 'RWF';
                    }else{
                        $currency_str = 'USD';
                    }
                    ...
                    switch($participant_data->pass_type){
                        case 'Silver':
                        case 'Gold':
                        case 'Platinum':
                    ...
                    // $currency = $currency_str;
                    // $orderInfo = 'ORDER34525';
                    $orderInfo = $participant_data->ID;// Order Info
                    $MerchTxnRef = $orderInfo.'-'.(Payment::generateMerchTxnRef()); // See functions.php file
                    ...
                    $queryData = array(
                        ...
                        'vpc_Version' => 1,
                        // 'vpc_ReturnURL' => 'http://127.0.0.1/bk-payment/return_url.php',
                        'vpc_ReturnURL' => (DN.'/groupsuccess/'.$participant_data->ID.'/success'),
...
<head>  
    ...
    <title>Payment - Transform Africa Summit 2018</title>
...
<body class="home page page-id-2" style="overflow: hidden">
	...
	<div id="wrapper" class="home"> 
        ...
        <section class="section-main " style="border-bottom: 1px solid #ddd; padding-bottom: 30px;">
         ...
         <?php 
            switch($request){
                case 'group':
                    ...
                    break;
                default:
                    include 'views/register/grouppayment-form.php';
                    break;
            }?>
        </section>
