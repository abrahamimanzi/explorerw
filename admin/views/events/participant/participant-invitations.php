              ...
              <div>
                ...
                <div class="box">
                    ...
                    <?php
                        ...
                        $participantTable = new RequestInvitation();
                        // $participantTable->selectQuery("SELECT * FROM `events_corporate` WHERE $search_sql ",$sql_val_array);
                        $participantTable->selectQuery("SELECT * FROM `events_request_invitation` ",$sql_val_array);
                        $participantApprovalNumber = $participantTable ->count();
                    ?>
                      <h3 class="box-title">Invitations Request <span style='color:#01aef0'><?=$participantApprovalNumber?></span></h3>
                          ...
                          <tbody>
                            ...
                            $seconds = \Config::get('time/seconds');
                              $participantTable = new RequestInvitation();
                                ...
                                $participantTable->selectQuery("SELECT* FROM `events_request_invitation` WHERE `pass_type`!='No pass' AND `state`!=? AND `event_ID`=? $search_sql ORDER BY {$_SESSION['participant_sort']}",$sql_val_array);
                            ...
                            if(Input::checkInput('request','post',1)){
                                ...
                                $participantTable->selectQuery("SELECT * FROM `events_request_invitation`  $search_sql ORDER BY {$_SESSION['participant_sort']} LIMIT {$offsetNumber},{$rowsLimit}",$sql_val_array);
