              ...
              <div>
                ...
                <div class="box">
                    ...
                    <?php
                        ...
                        $participantTable->selectQuery("SELECT * FROM `events_participant` WHERE  (`pass_type`='Silver') AND `state`!=? AND `event_ID`=? $search_sql ",$sql_val_array);
                        $participantApprovalNumber = $participantTable ->count();
                    ?>
                      <h3 class="box-title">Silver Delegate  <span style='color:#01aef0'><?=$participantApprovalNumber?></span></h3>
                        ...
                        <table class="table no-margin">
                          ...
                          <tbody>
                            ...
                            $seconds = \Config::get('time/seconds');
                              $participantTable = new Participant();
                                $participantTable->selectQuery("SELECT* FROM `events_participant` WHERE (`pass_type`='Silver') AND `state`!=? AND `event_ID`=? $search_sql ORDER BY {$_SESSION['participant_sort']}",$sql_val_array);
                            ...
                            if(Input::checkInput('keyword','get','0') || strlen(Input::get('keyword','get'))<3){
                                ...
                                $participantTable = new Participant();
                                $participantTable->selectQuery("SELECT * FROM `events_participant` WHERE (`pass_type`='Silver') AND `state`!=? AND `event_ID`=? $search_sql ORDER BY {$_SESSION['participant_sort']} LIMIT {$offsetNumber},{$rowsLimit}",$sql_val_array);
                    ...
                    <div class="box-footer clearfix">
                      <a href="<?=DNADMIN?>/app/EventApp/<?=$event_ID?>/participant-list" class="btn btn-sm btn-default btn-flat pull-right">View All Participants</a>
                        <?php  
                            ...
                            <a href="<?=DNADMIN?>/app/EventApp/<?=$event_ID?>/participant-list-exportsilver" class="btn btn-sm btn-primary btn-flat pull-right">Export</a>
                        <?php } ?>
                    </div>
                    <!-- /.box-footer -->
                  </div>
