...
{   
    ...
    public static function addSpecialAccountInviteCateg(){
        $diagnoArray[0] = 'NO_ERRORS';
        $validate = new \Validate();
        $prfx = 'subscriber-';
        foreach($_POST as $index=>$val){
        ...
        if($validation->passed()){
            ...
            $subscategInviteTable = new \SubscriberCategory();
            
            $str = new \Str();
            
            $_SIGNUP = (object)$_SIGNUP;

            $subscriber_ID = $str->data_in($_SIGNUP->subscriber_ID);

            $category_ID = $str->data_in($_SIGNUP->category_ID);

            $size = $str->data_in($_SIGNUP->size);

            $seen = $str->data_in($_SIGNUP->seen);

            $plan = $str->data_in($_SIGNUP->plan);

            $type = $str->data_in($_SIGNUP->type);
