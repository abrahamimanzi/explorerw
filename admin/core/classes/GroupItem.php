...
//		$user_ID = Session::get(Config::get('session/session_name'));
		...
		if(!$this->_db->insert('groups_item', $fields)){
...
// USER DATA UPDATE
	public function update($fields = array(),$id = null){
		if(!$this->_db->update('groups_item',$id,$fields)){
...
// FIND USER
	public function find($user = null,$limit = null){
		if($user){
			...
			if(is_numeric($user)){
				$field = 'ID';
				$data = $this->_db->get('groups_item', array($field, '=', $user),$limit);
				if($data->count()){
	...
	public function select($sql = null){
		$data = $this->_db->query("SELECT* FROM `groups_item` {$sql}");
		if($data->count()){
