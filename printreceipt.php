...
if(Input::checkInput('reference','get','1')){
    ...
    if($participant_item_data->payment_state == "Confirmed" OR $participant_item_data->payment_state == "Approved"){
...
//             Page footer
            function Footer(){
                ...
                $this->Image(DN.'/img/banner-01-2-old.jpg',0,710,600);
        ...
        // }
        if ($participant_item_data->card_issue == 'VC') {
            $card_issue = 'Visa Card';
        }else{
            $card_issue = 'MastrerCard';
        }
...
//        $pdf->SetY(358);
        ...
        $pdf->SetFont('Arial','B',10);
        if ($currency == 'USD') {
            $amount_total = chop($amount_total,"00");
            // $pdf->Cell(0,35,'Total: '.$currency.' '.$amount_total,0,0,'R'); 
            $pdf->Cell(0,64,'Paid: '.$currency.' '.$amount_total,0,0,'R');
        }else{
