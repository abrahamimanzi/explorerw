...
if(Input::checkInput('id','get','1')){
    ...
    switch($request){
        ...
        default:
            $participant_ID = $_id;
            $participantTable = new \Participant();
            $participantTable->selectQuery("SELECT `code`,`payment_rn`,`payment_method`,`firstname`,`lastname`,`othername`,`email`,`amount`,`payment_state`,`category`,`pass_type`,`state`,`invite_ID`, `residence_country` FROM `events_participant` WHERE `code`=? ORDER BY `ID` DESC LIMIT 1",array($participant_ID));
            ...
            switch($request){
                case 'group':
                    ...
                    $queryData = array(
                        ...
                        'vpc_ReturnURL' => (DN.'/success/'.$session_subscriber_data->group_ID.'/success'),
